...
// Evaluation service for grading exam submissions using Gemini 3 Pro Client-Side
export const evaluateExam = async (
  ...
  if (!apiKey) {
    console.error("API Key is missing. Please set VITE_GEMINI_API_KEY or VITE_API_KEY.");
    return createFallbackResult(questions, totalMaxScore, "Configuration Error: API Key missing.");
  }
...
// Simulation of code execution using our Gemini 3 Pro
export const executeCodeWithAI = async (code: string, language: string): Promise<{type: 'output' | 'error', content: string}> => {
  ...
  const apiKey = getApiKey();
  if (!apiKey) {
    return { type: 'error', content: "Configuration Error: API Key missing. Please set VITE_GEMINI_API_KEY." };
  }
